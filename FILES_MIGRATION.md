...
Статус: **план. Решения §3 (D1–D6) — НА ПОДТВЕРЖДЕНИИ, кода нет.**
Ветка (предлагается) `feat/attachments`. Порядок фаз: 1 → 2 → 3 → 4 → 5
...
Ниже — предлагаемые ответы на 4 вопроса из задания плюс два, которые всплывают
по ходу (D5 — как физически отдаётся файл и почему это и есть «наследование
видимости»; D6 — вложения только у задач, не у комментариев). Каждое — рекомендация
+ отклонённые альтернативы. **Ничего не реализуется до подтверждения.**
...
1. **D1** — согласны на «абстракция + оба драйвера, `local` по умолчанию, `s3` в
   CI и как прод-рекомендация»? Или строго один вариант (`local`-only /
   `s3`-only)?
2. **D2** — загрузка по `comment`, удаление своего всегда / чужого по `delete`,
   без нового ключа в MATRIX — ок? Или завести `manageAttachments`?
3. **D3** — 25 MiB дефолт, чёрный список расширений + magic-байты + флаг
   несовпадения, свой мини-детект без зависимости — ок? Другой лимит по размеру?
   Нужен ли всё-таки белый список для части типов?
4. **D3** — потолок `ATTACH_MAX_PER_ISSUE = 50` на задачу — ок?
5. **D5** — по умолчанию **прокси-стрим через API** (не presigned), заголовки
   `attachment` + `nosniff`, active-типы → `octet-stream` — ок?
6. **D6** — вложения только у задач в MVP (не у комментариев) — ок?
7. **Именование** — `FILES_MIGRATION.md` + `STORAGE_SETUP.md`, ветка
   `feat/attachments` — ок?
8. **Порядок со следующим этапом** — после вложений сразу
   `NOTIFICATIONS_MIGRATION.md` (план), затем реализация — подтверждаете?
