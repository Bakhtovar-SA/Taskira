...
Ветка (предлагается) `feat/notifications`. Порядок фаз: 1 → 2 → 3 → 4 → 5
...
Ниже — предлагаемые ответы. Каждое — рекомендация + отклонённые альтернативы.
**Ничего не реализуется до подтверждения.**
...
(для ссылок; без него email-ссылок нет — только текст). Шаблоны — RU, плоский
текст + лёгкий HTML, одна ссылка на задачу. Дайджест — одно письмо со списком.
Тема: «[CORP-123] вас назначили исполнителем» и т.п.

**Отклонено.** Тяжёлый шаблонизатор/вёрстка писем (MVP не нужно); внешний
email-API (SendGrid и пр. — on-prem, свой SMTP).
...
- **Тесты** — `server/test/notifier.test.ts` (job `mail`, только при
  ...
  /api/v1/messages`): адрес, тема `[CORP-N]`, ссылка; ретрай при недоступном SMTP
  (`email_tries++`, потом `failed`); дайджест — одно письмо на N событий;
  `email='off'` → `skipped`, письма нет; локальный юзер без `email` → `skipped`.
...
- **Персональные данные в `payload`.** Отрывок текста задачи/комментария уходит в
  email во внешний SMTP-релей — согласовать с ИБ (в NOTIFICATIONS_SETUP.md
  предупреждение); опция «только ссылка, без содержимого» — Фаза 6.
...
1. **D1** — in-app всегда + email опционально за `SMTP_*` (+ Mailpit в CI) — так?
   Или email обязателен / только in-app?
2. **D2** — набор триггеров (назначение, комментарий по подписке, `@`-упоминание,
   смена статуса, подключение collaborator, изменение состава проекта) — полный?
   Что убрать/добавить?
3. **D3** — одна таблица `notifications` с `email_state` (без отдельного outbox);
   in-app-доставка **по polling**, WebSocket — в Фазу 6 — согласны?
4. **D4** — фоновый воркер **в основном процессе** (`setInterval`, флаг
   `NOTIFY_WORKER_ENABLED` под будущее вынесение) — ок? Или сразу отдельный
   `npm run worker`?
5. **D5** — `@`-упоминания парсит **сервер**, резолвит только среди видящих
   задачу; клиентский автокомплит — Фаза 6 — ок?
6. **D6** — настройки MVP: только `email` (сразу/дайджест/выкл) + `selfWatch`,
   per-type — Фаза 6 — достаточно?
7. **D7** — `nodemailer`, RU plain-text + лёгкий HTML, ссылка через `APP_BASE_URL`
   — ок?
8. **Именование** — `NOTIFICATIONS_MIGRATION.md` + `NOTIFICATIONS_SETUP.md`, ветка
   `feat/notifications` — ок?
9. **Персональные данные** — можно ли включать отрывок текста задачи/комментария
   в тело письма (уходит во внешний SMTP), или письма «только ссылка» с самого
   начала?
