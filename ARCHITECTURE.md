...
- **Real-time доска** — `issue:upsert`/`issue:delete`/`workflow:changed`/`presence`
  из `WsMessage` остаются объявленными, но нереализованными: живые изменения
  доски между открытыми у разных людей вкладками по-прежнему требуют
  обновления страницы. Это осознанно отдельный, значительно больший объём
  работы, чем push уведомлений (см. ниже) — комнаты на проект, broadcast
  многим одновременно, re-render доски на чужие правки.
...
✅ `docker-compose.yml` (корень) поднимает PostgreSQL + API + клиент одной
командой ([DOCKER_SETUP.md](DOCKER_SETUP.md)) — но это не сам reverse proxy:
внешний nginx с TLS из списка выше по-прежнему ставится отдельно, `client` и
`server` просто публикуют порты, как в dev-режиме.
...
6. 🟡 Переименование сущностей в UI под нейтральную терминологию (см. SCOPE.md)
   ...
   точечное исключение из этого решения для отдельных проектов, не его отмена.

## UI и Content-Security-Policy: инлайн-стилей нет

`nginx.conf` отдаёт клиент с CSP `style-src 'self'; style-src-attr 'none'` —
inline-стили запрещены. Поэтому приложение собрано на собственном JSX-рантайме
(`src/secure-jsx/`, `jsxImportSource: "#secure-jsx"`): `style={{ … }}` превращается
в класс `taskira-dyn-N`, правило которого вставляется во внешний лист
`#taskira-dynamic-styles` (`dynamicStyle.ts`). Следствия для любых правок UI
(Трек 5):

- `element.style.width` читается пустым, а атрибута `style` в DOM нет: размеры
  в тестах и автоматизации меряйте по `getBoundingClientRect()`.
- Значения, содержащие `{ } ; @` или `url(...)`, отбрасываются.
- Каждому *различному* набору значений — отдельное CSS-правило (кэшируется по
  тексту). Непрерывно меняющиеся значения (позиция при перетаскивании, анимация
  через `style`) плодят правила без ограничения: для них нужны CSS-переменные
  или готовые классы, а не `style`.
- Устанавливать стили через DOM API (`el.style.setProperty`) нельзя — блокирует CSP.
