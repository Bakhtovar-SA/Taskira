...
export async function seedAdmin(): Promise<void> {
  ...
  const initials = cfg.admin.name
    ...
    .join("") || "АД";
  // global_role='admin' (миграция 004) — полный доступ во всех проектах через
  // глобальную роль; строку в project_members первому админу НЕ создаём
  // (правило ROLE_MIGRATION.md §3.3).
  await q(
