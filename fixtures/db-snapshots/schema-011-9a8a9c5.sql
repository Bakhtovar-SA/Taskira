-- Taskira upgrade snapshot through 011_notifications.sql.
...
CREATE TABLE public.issues (
    ...
    CONSTRAINT issues_points_check CHECK (((points >= 0) AND (points <= 100))),
    CONSTRAINT issues_priority_id_check CHECK ((priority_id = ANY (ARRAY['highest'::text, 'high'::text, 'medium'::text, 'low'::text, 'lowest'::text]))),
...
CREATE TABLE public.sprints (
    ...
    end_date date,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT sprints_goal_check CHECK ((char_length(goal) <= 200)),
...
INSERT INTO public.upgrade_snapshot_probe(value) VALUES ('011_notifications.sql');
