...
Статус: миграция завершена — Фазы 1–5 (схема, сервер, клиент, админ-UI, переключатель проектов). Требует браузерной проверки UI (Фазы 4–5).
...
### Фаза 5 — Переключатель проектов  *(сделано)*

**`src/components/Topbar.tsx`** — `ProjectSwitcher` в хлебных крошках: при
`data.projects.length > 1` имя проекта становится дропдауном (`Dropdown` +
`MenuItem` из `ui.tsx`) со списком видимых проектов (ключ + название, метка
«общий», галочка на текущем); выбор → `switchProject(id)` (уже был в store:
`buildProjectData` для нового проекта + `writeLastProject`). При одном проекте —
просто текст, как раньше.

`npm run typecheck` — новых ошибок нет; `npm run build` — успешно. UI требует
браузерной проверки (AdminView, ProjectSwitcher рендерятся, серверные вызовы
покрыты тестами).
